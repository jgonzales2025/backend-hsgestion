...
class CobranzaDetalleExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithEvents, ShouldAutoSize
{
    ...
    public function collection(): Collection
    {
        ...
        return collect($this->data)->map(function ($item) {
            // Convertir objeto a array usando JSON
            if (is_object($item)) {
                $item = json_decode(json_encode($item), true);
            } elseif (!is_array($item)) {
                $item = (array) $item;
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                // A5 es la fila de encabezados de la tabla
                if ($highestRow >= 5) {
